...
from .SeqUtil import Helper
from .FileUtil import read_file, write_file
...
class SeqExtracter(object):
    ...
    def __init__(
        self, _dir, config, project_by_label_table, multiprocess_log_queue=None
    ):
        """"""
        ...
        self.project_by_label = project_by_label_table
    ...
    def run(self, num_threads):
        ...
        # Make out & stat directories
        os.makedirs(os.path.join(self.dir, "out"), exist_ok=True)
        os.makedirs(os.path.join(self.dir, "stat"), exist_ok=True)
        ...
        if self.platform == "miseq" or self.platform == "novaseq":
            ...
            # self.filter_len = 200
            logging.info(
                "Platform: %s... Setting filtering length as 250" % self.platform
            )
        elif self.platform == "nextseq":
            self.filter_len = 150
            logging.info(
                "Platform: %s... Setting filtering length as 150" % self.platform
            )
        else:
        ...
        while True:
            ...
            target_project = self.project_by_label[tmp_name]
            os.makedirs(os.path.join(self.dir, "out", target_project), exist_ok=True)

            # (210520 -> 210622 -> 230227). for dealing with the problem of sample name in csv files
            tmp_name_components = tmp_name.replace("_", "-").split("-")
            for tmp_name_component in tmp_name_components:
                try:
                    int(tmp_name_component)
                except:
            ...
            self.label_dir = os.path.join(
                self.dir, "out", target_project, self.target_name
            )
    ...
    def extract_chunk_start_pos(self, target_line_len, chunk_size):
        ...
        logging.info(
            "Chunk size for target %s, distributed into %d processes: %d"
            % (self.target_name, self.num_process, chunk_size)
        )
    ...
    def write_csv(self):
        ...
        # write raw data file
        with open(
            os.path.join(self.label_dir, "%s.csv" % self.target_name), "w"
        ) as handle:
            write_file(handle, raw_header, list(self.rawCSV.values()))

        # write whole statistics file
        stat_outfile = os.path.join(self.dir, "stat", "Statistics.csv")
        if not os.path.isfile(stat_outfile):
            ...
            with open(stat_outfile, "a+") as handle:
                handle.writelines(
                    [",".join([str(self.statCSV[h]) for h in stat_header]) + "\n"]
                )
...
class ExtractWorker(Process):
    ...
    def __init__(
        ...
        self.filter_q, self.filter_p = (
            int(filter_m.group(1)),
            float(filter_m.group(2)) / 100,
        )
    ...
    def extract_sequence(self, chunk_start_pos):
        ...
        self.statCSV_chunk = {
            "qfilter": self.filter_text,
            "merged_reads": count_merged,
            "q-filtered_reads": count_nfiltered,
        }
