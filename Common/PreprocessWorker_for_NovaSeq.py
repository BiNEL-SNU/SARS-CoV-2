...
class PreprocessWorker(object):
    ...
    def __init__(self, work_dir, sample_name_list, infile_suffix='', outfile_suffix='',
                 ...
                 step=None, multiprocess_log_queue=None, chain_type=ChainType.HUMAN_HEAVY, file_type='tsv', debug=False, rev_primer_trimming=True):
    ...
    def run(self, num_process):
        ...
        if hostname == 'JunhoLab' and set_num_process > 30:
            ...
            set_num_process = 50                # max number of process is limited to 50 (binel229 server)
        ...
        while True:
            ...
            if self.step == 1:
                ...
                if True:
                    ...
                    for i in range(set_num_process):
                        p = ErrorCorrecter(target_queue, chunk_size, sub_step, result_queue, self.log_queue, RAM_used_queue,
                                           ...
                                           rev_primer_trimming=self.rev_primer_trimming, chain_type=self.chain_type)
                ...
                if True:
                    ...
                    if self.debug:
                        outfile_dir = sample_dir
                        output_file = os.path.join(outfile_dir, '%s_%s_UMI_merged.tsv' % (self.sample_name_to_put, self.outfile_suffix))
                ...
                if True:
                    ...
                    for i in range(set_num_process):
                        p = ErrorCorrecter(target_queue, chunk_size, sub_step, result_queue, self.log_queue, RAM_used_queue,
                                           input_dict=UMI_merged_dict, dist_thresh=self.dist_thresh, rev_primer_trimming=self.rev_primer_trimming)
                        p.daemon = True
                        p.start()
                ...
                if True:
                    ...
                    if False:
                        subcluster_dict = {}
                        subcluster_file = os.path.join(in_dir, 'SubClusteredsave_%s.tsv' % self.outfile_suffix)
                        ...
                        for SubCluster in len_filtered_dict:
                            for Seq in len_filtered_dict[SubCluster]:
                                NewList.append({'UMI-number': SubCluster, 'seqNum': Seq['seqNum'], 'rcSum': Seq['rcSum'],
                                                'readcount': Seq['readcount'], 'full_NT': Seq['full_NT']})
                    else:
                        w_header = ['UMI-number', 'seqNum', 'rcSum', 'readcount', 'full_NT', 'rev_primer_len']
                        for SubCluster in len_filtered_dict:
                            for Seq in len_filtered_dict[SubCluster]:
                                NewList.append({'UMI-number': SubCluster, 'seqNum': Seq['seqNum'], 'rcSum': Seq['rcSum'],
                                                'readcount': Seq['readcount'], 'full_NT': Seq['full_NT'], 'rev_primer_len': Seq['rev_primer_len']})
                    with open(os.path.join(sample_dir, 'LengthFiltered_%s.tsv' % self.outfile_suffix), 'w') as handle:
                ...
                if True:
                    ...
                    p = IsotypeExtractor(target_file, sub_step, self.log_queue, RAM_used_queue,
                                         out_dir=out_dir, c_ref_file=self.c_ref_file, num_process=set_num_process, delim=self.delim, rev_primer_trimming=self.rev_primer_trimming)
                    p.daemon = True
                    p.start()
                    extract_workers.append(p)
...
class ErrorCorrecter(Process):
    ...
    def __init__(self, in_queue, chunk_size, sub_step, out_queue, log_queue, RAM_used_queue, input_file='', input_dict={}, _dir=None,
                 fwd_primer_list=None, rev_primer_list=None, input_UMI_list=[], dist_thresh=5, hamming_thresh=1, vote_thresh=0.6, rev_primer_trimming=True,
                 chain_type=None):
            ...
            while True:
                ...
                for reversePrimerMatch in reversePrimerMatchList:
                    ...
                    if reverseMatchResult != None:
                        ...
                        if len(UMI) in [14, 15, 13]:
                            newSequence = sequence[:-(reverseTrimNum - reverseMatchResult.span()[1])]
                            success = True
                            try:
                                UMISet[(UMI, line_start)].append({'UMI': UMI, 'full_NT': newSequence,
                                                                  'readcount': readcount, 'rev_primer_len': len_recog_rev_primer})
    ...
    def UMI_merge(self, key_start):
        ...
        for targetUMI in UMI_list:
            ...
            for i in range(len(targetUMI)):
                target_UMI_hash_dict[targetUMI[:i] + targetUMI[i + 1:]] = 0         # for 1 bp deletion case
                        ...
                        if self.rev_primer_trimming:
                            ...
                            try:
                                ClusteredUMISet[UMI + '-' + str(subClusterNum)].append({'seqNum': len(resultSeqList), 'rcSum': readcountSum,
                                                                                        ...
                                                                                        'full_NT': resultSeq['full_NT'][:-rev_primer_trim_len]})
                            except KeyError:
...
class IsotypeExtractor(Process):
    ...
    def run_blast(self, input_file, **kwargs):
        ...
        format_cmd = '%s -in %s -dbtype nucl -input_type fasta -out %s' % (
            os.path.join(blast_exe_path, 'makeblastdb'), os.path.join(db_dir, 'db_UMI.fasta'), os.path.join(db_dir, 'db_UMI'))
            ...
            while True:
                ...
                if line[:len('Query=')] == 'Query=':
                    ...
                    if new_key in result_dict:
                        ...
                        result_dict[new_key] = {'full_NT': vdj_seq, 'isotype': isotype, 'readcount': rc, 'umi_list': umi_list}
