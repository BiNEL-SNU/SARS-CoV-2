...
class PreprocessWorker(object):
    ...
    def __init__(self, work_dir, sample_name_list, infile_suffix='', outfile_suffix='',
                 ...
                 step=None, multiprocess_log_queue=None, chain_type=None, file_type='tsv', debug=False):
            ...
            if self.step == 1:
                ...
                if True:
                    ...
                    if self.debug:
                        ...
                        w_header = ['UMI', 'full_NT', 'readcount']
                        w_data = []
                ...
                if True:
                    ...
                    if self.debug:
                        ...
                        w_header = ['mergedUMI', 'UMI', 'full_NT', 'readcount']
                        w_data = []
                ...
                if True:
                    ...
                    for i in range(set_num_process):
                        p = ErrorCorrecter(target_queue, chunk_size, sub_step, result_queue, self.log_queue, RAM_used_queue,
                                           input_dict=UMI_merged_dict, dist_thresh=self.dist_thresh)
                        p.daemon = True
                        p.start()
                ...
                if True:
                    ...
                    # # set sequence_id
                    # row_num = 1
                    # for k in sorted_annotated_dict:
                    #     sorted_annotated_dict[k]['sequence_id'] = '%s-%d' % (self.sample_name_to_put, row_num)
                    #     row_num += 1
    ...
    def tmp_write(self, Sub_clustered, sample_dir):
        ...
        for SubCluster in Sub_clustered:
            for Seq in Sub_clustered[SubCluster]:
                NewList.append({'UMI-number': SubCluster, 'seqNum': Seq['seqNum'], 'rcSum': Seq['rcSum'], 'readcount': Seq['readcount'], 'full_NT': Seq['full_NT']})
        with open(os.path.join(sample_dir, 'SubClusteredsave_%s.tsv' % self.outfile_suffix), 'w') as handle:
            write_file(handle=handle, header=['UMI-number', 'seqNum', 'rcSum', 'readcount', 'full_NT'], data=NewList, delim='\t')
...
class ErrorCorrecter(Process):
    ...
    def __init__(self, in_queue, chunk_size, sub_step, out_queue, log_queue, RAM_used_queue, input_file='', input_dict={}, _dir=None,
                 fwd_primer_list=None, rev_primer_list=None, input_UMI_list=[], dist_thresh=5, hamming_thresh=1, vote_thresh=0.6):
                            ...
                            for forwardPrimerMatch in forwardPrimerMatchList:
                                ...
                                if forwardMatchResult != None:
                                    ...
                                    try:
                                        ...
                                        UMISet[(UMI, line_start)].append({'UMI': UMI, 'full_NT': newSequence, 'readcount': readcount})
                                    except KeyError:
                                        # UMISet[UMI] = [{'UMI': UMI, 'full_NT': newSequence, 'readcount': readcount}]
                                        UMISet[(UMI, line_start)] = [{'UMI': UMI, 'full_NT': newSequence, 'readcount': readcount}]
                                    break
                                else:
                                    continue
                            break
    ...
    def sub_cluster_UMI_data(self, key_start):
        mergedUMISet = deepcopy(self.input_dict)
        ClusteredUMISet = {}
            ...
            while (len(seqList) > 0):
                ...
                if readcountSum == 2 and len(resultSeqList) == 2:
                    ...
                    for resultSeq in resultSeqList:
                        try:
                            ClusteredUMISet[UMI + '-' + str(subClusterNum)].append({'seqNum': len(resultSeqList), 'rcSum': readcountSum, 'readcount': resultSeq['readcount'], 'full_NT': resultSeq['full_NT']})
                        except KeyError:
                            ClusteredUMISet[UMI + '-' + str(subClusterNum)] = [{'seqNum': len(resultSeqList), 'rcSum': readcountSum, 'readcount': resultSeq['readcount'], 'full_NT': resultSeq['full_NT']}]
...
class IsotypeExtractor(Process):
    ...
    def __init__(self, input_file, sub_step, log_queue, RAM_used_queue, out_dir,
                 c_ref_file=None, in_queue=None, out_queue=None, num_process=1, chunk_size=None, delim='\t'):
        Process.__init__(self)
