...
from Common.FileUtil import detach_format, TableData, TableHeader, read_file_fast, write_file
from Common.ProcessUtil import run_igblast, run_igblast_new
from Common.ProcessUtil import run_IMPre
...
class Helper(object):
    ...
    def extend_degenerate_dna(self, sequence: str):
        ...
        #     r.append("".join(i))
        r = [''.join(i) for i in product(*[self.IUPAC_code[j] for j in sequence.upper()])]
        return r
    ...
    def annotate_regions_old(self, chain_type: ChainType, sequence_list: List[str], seq_type: str = 'Ig', **kwargs) -> TableData:
        """
        :param chain_type: 
        :param sequence_list: 
        :return: 
        ...
        if chain_type in [ChainType.HUMAN_HEAVY, ChainType.HUMAN_LIGHT, ChainType.CHICKEN_HEAVY, ChainType.CHICKEN_LIGHT, ChainType.RAT,
                          ChainType.RABBIT, ChainType.MOUSE_BALBC, ChainType.MOUSE_C57BL6]:
            ...
            for sequence, blast_result in zip(sequence_list, result_list):
                ...
                if chain_type in [ChainType.HUMAN_HEAVY, ChainType.HUMAN_LIGHT, ChainType.RABBIT]:
    ...
    def annotate_regions(self, chain_type: ChainType, sequence_list: List[str], annotation_list: List[str], **kwargs) -> TableData:
        ...
        # run_igblast(chain_type, tp_query.name, file_blast, seq_type, domain_system, **blast_kwargs)
        run_igblast_new(chain_type, tp_query.name, file_blast, seq_type, domain_system, **blast_kwargs)
        ...
        for sequence, igblast_parsed_data in zip(sequence_list, result_list):
            ...
            for r in ['fr1', 'fr2', 'fr3', 'cdr1', 'cdr2']:
                if ('%s_from' % r) in igblast_parsed_data and ('%s_to' % r) in igblast_parsed_data:
                    ...
                    igblast_parsed_data[r + '_aa'] = self.translate(igblast_parsed_data[r])
                else:
    ...
    def annotate_regions_chicken_loose(self, seq_list, v_type, e_val=5, result_dir='/home/IP-team/data/blast_db/blast_temp/'):
        query_dir = os.path.join(result_dir, 'query')
        out_dir = os.path.join(result_dir, 'out')
                ...
                if chain_type in [ChainType.HUMAN_HEAVY, ChainType.HUMAN_LIGHT,
                    ...
                    while True:
                        ...
                        if line[:len('Query= ')] == 'Query= ':
                            ...
                            if chain_type in [ChainType.HUMAN_HEAVY, ChainType.RABBIT_HEAVY,
                                ...
                                if 'No hits found' in line:
                                    ...
                                    if len(j_cigar_mismatch) != 0 and len(germline_alignment) != 0:
                                        for p in range(len(j_cigar_mismatch)):
                                            germline_alignment = germline_alignment[: j_cigar_mismatch[p][0]] + j_cigar_mismatch[p][1] + germline_alignment[
                                    ...
                                    if (len(d_germ_start_where)) == 0 and (len(j_germ_finish_where)) != 0 and len(v_germ_finish_where) != 0:
                                        ...
                                        second_string = ''
                                        for u in range(
                                                seq_before_d_num + d_line_pos[0] - int(v_germ_finish_where[(len(v_germ_finish_where) - 1)]) + int(v_germ_start_where[0]) - 1):
                                            the_string = the_string + '-'
                                        for u in range(
                                    ...
                                    elif chain_type in [ChainType.HUMAN_LIGHT, ChainType.RABBIT_KAPPA,
    ...
    def parse_IMPre(self, sequence_list: List[str], readcount_list: List[int], seq_type: str = 'Ig', chain: str = 'heavy', **kwargs) -> TableData:
        """
        :param sequence_list:
        ...
        run_IMPre(query=tp_query.name, seq_type=seq_type, chain=chain, vn=vn, v_seed=v_seed, iter_num=iter_num)
        logging.info('-- Running IMPre finished')

        result_dir = '/Tools/IMPre-master/Out_temp/%s_vn=%d_vseed=%d_iter=%d' % (chain, vn, v_seed, iter_num)
        v_infer = open(os.path.join(result_dir, '%s_V_Germline.final.fa' % seq_type), 'r')
        j_infer = open(os.path.join(result_dir, '%s_J_Germline.final.fa' % seq_type), 'r')
    ...
    def blast_run(self, query_list: list(), db_list: list(), m_type: str, blast_dir: str = '/home/IP-team/data/blast_temp/', **kwargs):
        ...
        if m_type.upper() == 'AA':
            ...
            format_cmd = '%s -in %s -dbtype prot -input_type fasta -out %s' % (
                os.path.join(exe_path, 'makeblastdb'), os.path.join(db_dir, 'db.fasta'), os.path.join(db_dir, 'db'))
            os.system(format_cmd)

            cline = NcbiblastpCommandline(num_threads=num_threads, query=os.path.join(query_dir, 'query.fasta'), db=os.path.join(db_dir, 'db'),
                                          out=result_file,
                    ...
                    for alignment in each.alignments:
                        ...
                        for i, eMatch in enumerate(hsps.match):
                            if eMatch == ' ':
                                ...
                                matchPattern += '|'
                        alignmentList.append({'matched': matched, 'whole': whole, 'sbjct_index': sbjctIndex,
                                              'query_start': queryStart, 'query_end': queryEnd,
                                              'sbjct_start': sbjctStart, 'sbjct_end': sbjctEnd,
                    ...
                    for alignment in each.alignments:
                        ...
                        alignmentList.append(
                            {'matched': matched, 'whole': whole, 'sbjct_index': sbjctIndex, 'query_start': queryStart,
                             ...
                             'num_mismatches': numMismatches, 'mismatches': mismatchList, 'match_pattern': matchPattern})
                    tempSet = {'num_alignments': numAlignments, 'alignments': alignmentList}
                    resultList.append(tempSet)
            return resultList
