...
class PESParser(object):
    ...
    def run(self, num_threads):
        ...
        for m in target_matches:
            ...
            elif len(sample_list) >= 3 and sample_list[-2][0] == 'S' and sample_list[-1][0] == 'L':
            ...
            if sampleID not in name_overlap_check:
                # sampleName = m.group(1)
                # sampleID = sampleName.split('_')[0]
                # name_overlap_check[m.group(1)] = 1
                name_overlap_check[sampleID] = 1
            else:
                # # sampleName = m.group(1) + '_add%d' % name_overlap_check[m.group(1)]
                # sampleName = m.group(1)
                # sampleID = sampleName.split('_')[0] + '_add%d' % name_overlap_check[sampleName.split('_')[0]]
            ...
            target = (sampleID, m.string[:m.start(2)] + 'R1' + m.string[m.end(2):],
    ...
    def _quailty_check(self, directory: str, target, assem_file: str):
        ...
        if Path(stat_file).is_file():
            ...
            if total == 0:
                logging.info('%s file is empty.. Pass _quality_check process.' % os.path.basename(stat_file))
                return
