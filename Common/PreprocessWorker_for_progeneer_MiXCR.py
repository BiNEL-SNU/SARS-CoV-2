...
#           (3) applying MiXCR, not UMI processing
#           (4) primer recognition using blast (not pattern matching methodology)
#           version 1.2
#
#####################################################################################################
    ...
    def run(self, num_process):
        ...
        while True:
            ...
            if self.step == 1:
                ...
                ##################################################################################
                ######## sub-step: 1a-1. run blast on fwd/rev primer sequences ###################
                ##################################################################################
                if True:
                    sub_step = 'a-1'
                    ...
                    extract_workers = []
                    p = ErrorCorrecter(sub_step, self.log_queue, RAM_used_queue,
                                       input_file=target_file, _dir=out_dir, fwd_primer_list=self.fwd_primers, rev_primer_list=self.rev_primers,
                                       rev_primer_trimming=self.rev_primer_trimming, num_process=set_num_process)
                    p.daemon = True
                    p.start()
                    ...
                    logging.info('2a. Run blast on fwd/rev primers was done.')
                    logging.info('--- %s seconds elapsed ---' % (datetime.now() - start_time))

                ##################################################################################
                ######## sub-step: 1a-2. parse blast results and get primer recognized seqs ######
                ##################################################################################
                if True:
                    sub_step = 'a-2'
                    RAM_used_queue = manager.Queue()  # RAM_used queue
                    ...
                    blast_result_dir = os.path.join(out_dir, 'temp_blast', 'result')
                    ...
                    ############################## started MP #########
                    logging.info("Do parse primer recognition info (step 1a-2) of ErrorCorrecter with %d processes", set_num_process)
                    extract_workers = []

                    for i in range(set_num_process):
                        p = ErrorCorrecter(sub_step, self.log_queue, RAM_used_queue, in_queue=target_queue, out_queue=result_queue,
                                           chunk_size=chunk_size, input_file=target_file, _dir=blast_result_dir, rev_primer_trimming=self.rev_primer_trimming)
                        p.daemon = True
                        p.start()
                    ...
                    primer_recognized_dict = self.merge_chunk_dict_type2(in_result_queue=out_result_queue, rc_col_name='readcount')

                    # temporary file write for the comparison
                    if self.debug:
                        outfile_dir = sample_dir
                        output_file = os.path.join(outfile_dir, '%s_%s_primer_recognized_by_blast.csv' % (self.sample_name_to_put, self.outfile_suffix))
                    ...
                    logging.info('1a-2. Parsing primer recognized reads was done.')
                    logging.info('--- %s seconds elapsed ---' % (datetime.now() - start_time))

                ##################################################################################
                ######## sub-step: 1b. apply MiXCR -> no multiprocessing manner ##################
                ##################################################################################
                if True:
                    ...
                    extract_workers = []
                    p = ErrorCorrecter(sub_step, self.log_queue, RAM_used_queue, out_queue=result_queue,
...
class ErrorCorrecter(Process):
    ...
    def __init__(self, sub_step, log_queue, RAM_used_queue, in_queue='', out_queue='', chunk_size='', input_file='', input_dict={}, _dir=None,
                 fwd_primer_list=None, rev_primer_list=None, evalue=0.001, rev_primer_trimming=True, num_process=1):
    ...
    def run(self):
        ...
        if self.sub_step == 'a-1':
            ...
            targetHeader, targetData = read_file_fast(self.input_file, delim=',')
            ...
            while True:
                ...
                # extract primer_recognized sequences
                self.parse_primer_recognized(chunk_start_pos, targetData)
                self.in_queue.task_done()
    ...
    def run_blast(self, **kwargs):
        ...
        targetSetHeader, targetSeqList = read_file_fast(self.input_file, delim=',')

        # directory setting
        if not self.dir:
            output_dir = '/'.join(os.path.dirname(self.input_file).split('/') + ['1_error_correction'])
        else:
            output_dir = self.dir
        blast_dir = os.path.join(output_dir, 'temp_blast')
        ...
        if True:
            ...
            fwd_db_dict = {'fwd_%d' % num:fwd_primer for (num, fwd_primer) in enumerate(forwardPrimerList)}
            rev_db_dict = {'fwd_%d' % num:rev_primer for (num, rev_primer) in enumerate(reversePrimerList)}
            fwd_result_file_name = 'fwd_blasted_MiXCR.txt'
            rev_result_file_name = 'rev_blasted_MiXCR.txt'

        fwd_result_file = os.path.join(result_dir, fwd_result_file_name)
        rev_result_file = os.path.join(result_dir, rev_result_file_name)
        ...
        with open(os.path.join(db_dir, 'fwd_db_MiXCR.fasta'), 'w') as fasta_writer:
            for name, e_seq in fwd_db_dict.items():
                fasta_writer.write('>' + name + '\n')
                fasta_writer.write(e_seq + '\n')
        with open(os.path.join(db_dir, 'rev_db_MiXCR.fasta'), 'w') as fasta_writer:
            for name, e_seq in rev_db_dict.items():
                fasta_writer.write('>' + name + '\n')
                fasta_writer.write(e_seq + '\n')

        # construct db
        format_cmd = '%s -in %s -dbtype nucl -input_type fasta -out %s' % (
            '/Tools/ncbi-blast-2.7.1+/bin/makeblastdb', os.path.join(db_dir, 'fwd_db_MiXCR.fasta'), os.path.join(db_dir, 'fwd_db_MiXCR'))
        os.system(format_cmd)
        format_cmd = '%s -in %s -dbtype nucl -input_type fasta -out %s' % (
            '/Tools/ncbi-blast-2.7.1+/bin/makeblastdb', os.path.join(db_dir, 'rev_db_MiXCR.fasta'), os.path.join(db_dir, 'rev_db_MiXCR'))
        os.system(format_cmd)

        # run blastn
        cline = NcbiblastnCommandline(num_threads=self.num_process, query=os.path.join(query_dir, 'query_MiXCR.fasta'), db=os.path.join(db_dir, 'fwd_db_MiXCR'),
                                      evalue=evalue, out=fwd_result_file, gapopen=gapopen, gapextend=gapextend, word_size=word_size,
        ...
        cline = NcbiblastnCommandline(num_threads=self.num_process, query=os.path.join(query_dir, 'query_MiXCR.fasta'), db=os.path.join(db_dir, 'rev_db_MiXCR'),
                                      evalue=evalue, out=rev_result_file, gapopen=gapopen, gapextend=gapextend, word_size=word_size,
                                      num_descriptions=num_alignments, num_alignments=num_alignments)
        format_exe = '/Tools/ncbi-blast-2.7.1+/bin/blastn'
        os.system(format_exe + str(cline)[len('blastn'):])
    ...
    def parse_primer_recognized(self, key_start, input_data_list):
        helper = Helper()

        result_dict = {}
        fwd_blast_result_file = os.path.join(self.dir, 'fwd_blasted_MiXCR.txt')
        rev_blast_result_file = os.path.join(self.dir, 'rev_blasted_MiXCR.txt')

        tmp_result_dict = {}

        # read a fwd primer blast result file and parse info
        with open(fwd_blast_result_file, 'r') as handle:
            key_start_count = 0
            ...
            while True:
                ...
                if line[:len('Query=')] == 'Query=':
                    ...
                    # user-defined blast parser (specific purpose: extract required info for primer recognition)
                    while True:
                        line = handle.readline()
                        if 'No hits found' in line:
                            break
                        elif line[:len('Length')] == 'Length':
                            fwd_len = int(line.split('=')[-1].strip())
                            info_accessed += 1
                            ...
                            matched = int(matched)
                            whole = int(whole)
                            gap = int(line.split(',')[1].split('=')[-1].strip().split()[0].split('/')[0])
                            info_accessed += 3
                            ...
                            if direction == 'Plus/Plus':
                                ...
                                trim_pos = query_end
                                info_accessed += 1
                            elif direction == 'Plus/Minus':
                                query_start = int(line.split()[1].strip())
                                trim_pos = query_start
                                info_accessed += 1
                    ...
                    numMismatches = fwd_len - matched
                    full_diff = numMismatches + gap
                    if int(full_diff) > 8:
                        continue

                    # save at temp_result_dict
                    tmp_result_dict[access_num] = {'direction': direction, 'trim_pos': {'fwd': trim_pos}}

        # read a rev primer blast result file and parse info
        with open(rev_blast_result_file, 'r') as handle:
            key_start_count = 0
            accessed_key_count = 0
                ...
                if line[:len('Query=')] == 'Query=':
                    ...
                    while True:
                        ...
                        if 'No hits found' in line:
                            ...
                            rev_len = int(line.split('=')[-1].strip())
                            info_accessed += 1
                        elif line.strip()[:len('Identities')] == 'Identities':
                            identities = line.split(',')[0].split('=')[-1].strip().split()[0]
                            matched, whole = identities.split('/')
                    ...
                    # pass not well aligned cases
                    numMismatches = rev_len - matched
                    full_diff = numMismatches + gap
                    if int(full_diff) > 8:
                        continue

                    # save at temp_result_dict
                    if access_num in tmp_result_dict:
                        tmp_result_dict[access_num]['trim_pos']['rev'] = trim_pos
                    else:
                        continue

        # trim primer recognized region and save into result_dict
        for access_num in tmp_result_dict:
            if len(tmp_result_dict[access_num]['trim_pos']) < 2:
                continue
            direction = tmp_result_dict[access_num]['direction']
            seq = input_data_list[access_num]['full_NT']
            rc = input_data_list[access_num]['readcount']

            if direction == 'Plus/Plus':
                trim_start = tmp_result_dict[access_num]['trim_pos']['fwd']
                trim_end = tmp_result_dict[access_num]['trim_pos']['rev']
                if self.rev_primer_trimming:
                    trim_seq = seq[trim_start:(trim_end-1)]
                else:
                    trim_seq = seq[trim_start:]

            elif direction == 'Plus/Minus':
                trim_start = tmp_result_dict[access_num]['trim_pos']['rev']
                trim_end = tmp_result_dict[access_num]['trim_pos']['fwd']
                if self.rev_primer_trimming:
                    trim_seq = seq[trim_start:(trim_end-1)]
                else:
                    trim_seq = seq[:(trim_end-1)]
                trim_seq = helper.to_reverse_complement(trim_seq)

            if trim_seq not in result_dict:
                result_dict[trim_seq] = {'full_NT': trim_seq, 'readcount': rc}
            else:
                result_dict[trim_seq]['readcount'] += rc
...
class IsotypeExtractor(Process):
    ...
    def run_blast(self, input_file, **kwargs):
        ...
        # construct db
        format_cmd = '%s -in %s -dbtype nucl -input_type fasta -out %s' % (
            '/Tools/ncbi-blast-2.7.1+/bin/makeblastdb', os.path.join(db_dir, 'db_MiXCR.fasta'), os.path.join(db_dir, 'db_MiXCR'))
        os.system(format_cmd)
